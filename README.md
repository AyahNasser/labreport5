...
- Here, we can see the difference between the two examples; where in the first one, the file was viewed without the line numbers. However, in the second example, the file was viewed with the line numbers displayed of the file content. 
  
 - The -N option in the less command is very useful and helpful because it helps you to keep track of the content of large files. Especially when dealing and debugging with large code files.
...
- This command-line is helpful because it helps us start the text file from a specific place. It is also very useful in large files where if someone knows the word or the line they are searching for where this command will just save time by locating the file from that line of the word without having to scroll through the file or trying to find a string. Also, I found this interesting and helpful information from the lab tasks and trying the command-line by myself and chatGPT. 
 

- 3) -i
This command line -i option is a kind of command line option that ignores the case sensitivity when searching for a pattern. This command line is helpful when searching for something in a file, and you want all the words in lower and upper case because this command line will not distinguish between uppercase and lowercase. 
...
<img width="573" alt="image" src="https://user-images.githubusercontent.com/122571192/224864585-9ac05652-7722-4457-84a4-153ba3221256.png">
 Although here the word "Small" appears to be in lowercase letters in the text file, however, I was still able to find it using the option of ignoring the case sensitivity. 

- b) The second example is trying to find the same word using the same command line, but I will use the option of taking case sensitivity into account. 
-The output:
...
- When I make it sensitive for cases, it will not find the word small, although we know it exists in the file from the last example; since it is case sensitive, it will only match the exact words. 
- This command-line is helpful because it helps you quickly find a certain pattern in a file without having to go through the whole file line by line, which again saves time and effort. Also, I found this interesting and helpful information from the lab tasks and trying the command-line by myself and chatGPT. 
 



- 4) -S
This command-line is an option with less command, and it helps to view the very long lines that might be wrapped into multiple lines because they cannot fit in the width of the terminal window. 
- a) First Example, I am trying to view the text file WhereToMadrid, which has longer lines that cannot fit in the width of my screen.
